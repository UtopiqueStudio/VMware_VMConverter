...
namespace VMConverter {
    ...
    class ConverterConnection {
      ...
      public InstallAgentResult InstallAgent(String sourceName, int sourceAgentPort, String sourceThumbprint, String sourceUsername, String sourcePassword, bool sourcePostponeReboot) {

         // Check for Agent existence.
         ConverterComputerSpecLiveComputerLocation liveComputerLocation = Common.BuildLiveSourceLocation(sourceName, sourceUsername, sourcePassword, "windowsOs", sourceThumbprint);
         ...
         do {
            ...
            try {
               ...
               if (remoteThumbprint != null) {
                  ...
                  if (liveComputerLocation.sslThumbprint == null || liveComputerLocation.sslThumbprint.Length == 0) {
         ...
         try {
            ...
            ConverterAgentManagerAgentDeploymentResult result = _converterService.ConverterInstallAgent(_converterServerContent.agentManager,
            ...
            } else if (result.status == ConverterAgentManagerDeploymentStatus.rebootRequired && sourcePostponeReboot == true ) {
               Console.WriteLine("A reboot of the physical source {0} is required for the agent installation to succeed. Please try the P2V after rebooting the source", sourceName);

               return new InstallAgentResult(false, result.sslThumbprint);
            } else if (result.status == ConverterAgentManagerDeploymentStatus.rebootRequired && sourcePostponeReboot == false) {
               _converterService.ConverterRebootMachine(_converterServerContent.agentManager, sourceName, sourceUsername, sourcePassword);
      ...
      ReadYesNo(String prompt, bool defaultValue) {
         ...
         do {
            ...
            if (line.CompareTo("y") == 0 || line.CompareTo("yes") == 0) {
               result = true;
               ready = true;
            } else if (line.CompareTo("n") == 0 || line.CompareTo("no") == 0) {
               result = false;
               ready = true;
            } else if (line.CompareTo("c") == 0 || line.CompareTo("cancel") == 0) {
               // use default answer
               ready = true;
            }
